require([
  ...
  $('#compare').on('click', function () {
    if ($('#Missions').attr('Status') === '1') {
      ...
      if (parseInt($(this).attr('status')) == '1') {
        ...
        $('#referenceWindow').attr('id', 'referenceWindow1')
        $('#targetWindow').attr('id', 'targetWindow1')
        $('#main').css({ 'margin-right': '138px', 'margin-left': '138px' })
        $(this).attr('status', '2')
      } else if (parseInt($(this).attr('status')) == '2') {
        $('#years').css('display', 'none')
        $('#years_right').css('display', 'none')
  ...
  $('#landform').on('click', function () {
    ...
    $('#years>#5ec29361342b489b8c975b8674166bdc').click()
    $('#years_right>#b8cd9a3efc4a4f1ba6e889b54e74965d').click()
    $('#4').click()
  })
